...
function answerInfinity() {
    ...
    if (name === pokemonName[String(pokedexNumber[pokedexNumber.length - 1])]) {
        // 正解処理
        correctCount++;
        document.getElementById("content_01").textContent = `正解数：${correctCount}/${infinityCount}　`;
    } else {
        // 不正解処理
        document.getElementById("content_02").textContent = `不正解数：${incorrectCount}/${infinityCount}　`;
    ...
    // 全問回答したらresultページに遷移する
    if (questionsCount > infinityCount) {
        sessionStorage.setItem('index', JSON.stringify(pokedexNumber));
        sessionStorage.setItem('pokename', JSON.stringify(pokeName));
        sessionStorage.setItem('correct', correctCount);
        sessionStorage.setItem('incorrect', incorrectCount);
        let total_time = `${minutes}分${second}秒`;
        sessionStorage.setItem('time', total_time);
        window.alert(infinityExplanation);
        window.location = resultURL;
        return
    }
...
function init() {
    ...
    _showQuestion();
    // ボタンイベント登録
    document.getElementById("Answer").addEventListener("click", answerInfinity);
}
// ページロード時にinit関数を呼び出す
