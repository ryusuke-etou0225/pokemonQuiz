...
function answerNormal() {
    ...
    if (name === pokemonName[String(pokedexNumber[pokedexNumber.length - 1])]) {
        // 正解処理
        correctCount++;
        document.getElementById("content_01").textContent = `正解数：${correctCount}/15　`;
    } else {
        // 不正解処理
        incorrectCount++;
        document.getElementById("content_02").textContent = `不正解数：${incorrectCount}/15　`;
    }
...
function init() {
    ...
    _showQuestion();
    // ボタンイベント登録
    document.getElementById("Answer").addEventListener("click", answerNormal);
}
// ページロード時にinit関数を呼び出す
